...
function getPlatformTag(sourceUrl?: string) {
  ...
  if (url.includes("tiktok")) {
    return { name: "TikTok", badge: "bg-pink-500/10 text-pink-400 border-pink-500/30" };
  }
  if (url.includes("facebook") || url.includes("fb.watch")) {
    return { name: "Facebook", badge: "bg-blue-500/10 text-blue-400 border-blue-500/30" };
  }
  if (url.includes("instagram")) {
    return { name: "Instagram", badge: "bg-purple-500/10 text-purple-400 border-purple-500/30" };
  }
  if (url.includes("youtube") || url.includes("youtu.be")) {
    return { name: "YouTube", badge: "bg-red-500/10 text-red-400 border-red-500/30" };
  }
  return { name: "Social Media", badge: "bg-indigo-500/10 text-indigo-400 border-indigo-500/30" };
}
...
function getDynamicBadge(item: MediaOption) {
  ...
  if (quality.includes("hd_no_watermark") || (quality.includes("hd") && quality.includes("no_watermark"))) {
    return {
      ...
      style: "bg-purple-500/15 text-purple-300 border-purple-500/40 font-semibold",
    };
  }
          ...
          <div>
            ...
            {authorName && (
              <div className="mt-2.5 flex items-center gap-2 text-xs md:text-sm text-zinc-400">
                <span className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-lg bg-zinc-800/80 border border-zinc-700/40 text-zinc-200 font-medium">
...
function VideoResultSkeleton() {
  return (
    <div className="w-full bg-zinc-900/60 border border-zinc-800/80 rounded-2xl p-5 md:p-6 shadow-2xl animate-pulse flex flex-col gap-6">
